...
import useAdminCreateCampaignHook from '@/src/routes/Admin/Hooks/admin-create-campaign-hook';
...
import useAdminAllCompaniesNamesHook from '@/src/routes/Admin/Hooks/admin-all-companies-names-hook';

interface AdminSummaryPopupProps {
  onClose: () => void;
}
  ...
  return (
    <div className="dark:bg-white/5 rounded-lg p-3 lg:p-4 border border-slate-400 dark:border-white/10">
      <div className="flex items-center gap-2 mb-2 lg:mb-3">
        ...
        <h4 className="text-xs lg:text-sm font-semibold text-foreground dark:text-white">
          {config.label}
        </h4>
      </div>
            ...
            options.map((option) => {
              ...
              return (
                <DropdownMenuCheckboxItem
                  ...
                  className={`text-xs lg:text-sm text-foreground/95 focus:bg-white/10 ${config.color.text}`}
                >
                  {option}
                </DropdownMenuCheckboxItem>
              );
...
const companyFieldColor = {
  dot: 'bg-cyan-400',
  gradient: 'from-cyan-500/20 to-sky-500/20',
  border: 'border-cyan-400/30',
  text: 'text-foreground/80 dark:text-cyan-100',
  ring: 'ring-cyan-400/50',
};

export default function AdminSummaryPopup({ onClose }: AdminSummaryPopupProps) {
  const {
    platform,
    category,
    limit,
  ...
  } = useAdminAllCompaniesNamesHook();

  const { mutateAsync: createAdminCampaign, isPending } = useAdminCreateCampaignHook();
  ...
  const handleRemove = (config: FieldConfig, item: string) => {
    if (config.isArray) {
      removeFromArray(
        config.key as 'platform' | 'category' | 'followers' | 'country' | 'limit',
        item,
      );
    } else {
      setField(config.key, '');
  ...
  const handleLaunchCampaign = async () => {
    ...
    const limitNum = Number.isFinite(limitParsed) && limitParsed > 0 ? limitParsed : 10;

    const payload: AdminCreateCampaignRequest = {
  ...
  const isFormComplete =
    selectedCompanyName.trim().length > 0 &&
    platform?.length > 0 &&
    category?.length > 0 &&
    followers?.length > 0 &&
    country?.length > 0 &&
      ...
      <div className="relative w-full max-w-3xl lg:max-w-5xl xl:max-w-6xl my-8 pb-75">
        <div className="group rounded-3xl border border-white/20 bg-linear-to-br from-blue-200 to-red-200 dark:from-slate-900 dark:via-gray-900 dark:to-zinc-900 shadow-2xl backdrop-blur-sm p-6 md:p-8 lg:p-10">
          ...
          <div className="relative z-10">
            <div className="flex flex-col sm:flex-row items-start justify-between mb-6 lg:mb-8 gap-3">
              <div className="flex items-center gap-3 lg:gap-4">
                <div className="relative">
                  ...
                  <div className="relative flex h-12 w-12 lg:h-14 lg:w-14 items-center justify-center rounded-full bg-linear-to-r from-slate-500 to-gray-600 text-foreground/95 text-xl lg:text-2xl font-bold shadow-lg">
                    📋
                  </div>
                </div>
                <div>
                  <h3 className="text-lg md:text-xl lg:text-2xl font-bold text-foreground/95 mb-1 tracking-tight">
                    Campaign Summary
                  </h3>
                  <p className="text-foreground/70 text-xs lg:text-sm leading-tight">
                    Review settings and assign the campaign to a company before launching
                  </p>
                </div>
              </div>
              <button
            ...
            {/* Company name — loaded from GET /admin/all-companies-names */}
            <div className="mb-4 lg:mb-6">
              <div className="dark:bg-white/5 rounded-lg p-3 lg:p-4 border border-slate-400 dark:border-white/10">
                <div className="flex items-center gap-2 mb-2 lg:mb-3">
                  <div
                    className={`h-1.5 w-1.5 lg:h-2 lg:w-2 rounded-full ${companyFieldColor.dot}`}
                  />
                  <Building2 className="w-3.5 h-3.5 text-foreground/70" aria-hidden />
                  <h4 className="text-xs lg:text-sm font-semibold text-foreground dark:text-white">
                    Company name
                  </h4>
                </div>
                {companiesLoading ? (
                  <div className="flex items-center justify-center gap-2 py-6 text-foreground/70 text-sm">
                    <Spinner size={18} />
                    Loading companies…
                  </div>
                ) : companiesError ? (
                  <p className="text-sm text-red-400 py-2">
                    Could not load company list. Try again later.
                  </p>
                  ...
                  <DropdownMenu open={companyMenuOpen} onOpenChange={setCompanyMenuOpen}>
                    <DropdownMenuTrigger asChild>
                      <button
                        type="button"
                        className={`w-full flex items-center justify-between h-8 lg:h-10 px-3 lg:px-4 rounded-md text-xs lg:text-sm bg-white/5 border dark:border-b-white/10 border-b-slate-400 text-foreground dark:text-white/70 hover:bg-white/10 transition-colors ${companyFieldColor.ring} focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-900`}
                      >
                    ...
                    <DropdownMenuContent
                      className={`z-100 w-(--radix-dropdown-menu-trigger-width) max-h-62.5 overflow-y-auto bg-slate-800/95 backdrop-blur-sm border-white/10 ${companyFieldColor.border}`}
                      align="start"
                      side="bottom"
                      sideOffset={4}
                      avoidCollisions={false}
                    ...
                    >
                      <DropdownMenuRadioGroup
                        ...
                        onValueChange={(val) => setSelectedCompanyName(val)}
                      >
                        {companyNames.map((name) => (
                          <DropdownMenuRadioItem
                            key={name}
                            value={name}
                            className={`text-xs lg:text-sm text-foreground/95 focus:bg-foreground/10 ${companyFieldColor.text}`}
                          >
                            {name}
                          </DropdownMenuRadioItem>
                        ))}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-6">
              {fieldConfigs.map((config) => (
                <EditableField
                  ...
                  onRemove={(item: string | string[]) =>
                    handleRemove(config, item as string)
                  }
                />
              ))}
            </div>
            ...
            <div className="mt-6 lg:mt-8 space-y-3 lg:space-y-4">
              <div className="flex flex-col sm:flex-row gap-3 lg:gap-4">
                ...
                <CustomButton
                  ...
                  disabled={!isFormComplete || isPending || companiesLoading}
                  className={`flex-1 px-4 py-2.5 lg:py-3 rounded-lg text-sm lg:text-base font-medium transition-all duration-200 ${
                    isFormComplete
              ...
              <div className="text-center">
                <p className="text-xs lg:text-sm text-foreground/70">
                  {isFormComplete
                    ...
                    : 'Select a company and complete all fields above to launch'}
                </p>
              </div>
            </div>
          </div>
