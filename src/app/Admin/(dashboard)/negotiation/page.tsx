...
import { RefreshCcw, Handshake, Trash } from 'lucide-react';
        ...
        subheader={data?.negotiation_controls?.map(
          (userSession: NegotiationStatsResponse) => {
            const analysisIntent =
              userSession?.analysis && 'intent' in userSession.analysis
                ? userSession.analysis.intent
                : null;
            const analysisNextAction =
              userSession?.analysis && 'next_action' in userSession.analysis
            ...
            return [
              ...
              <div
                key={`user-message-${userSession._id}`}
              ...
              <div
                key={`next-action-${userSession._id}`}
                className="truncate"
                title={userSession?.next_action || analysisNextAction || ''}
              >
              ...
              <div
                key={`final-reply-${userSession._id}`}
                className="truncate max-w-xs"
                title={userSession?.final_reply || ''}
              >
