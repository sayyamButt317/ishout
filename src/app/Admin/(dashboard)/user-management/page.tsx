                ...
                {(['all', 'admin', 'company'] as const).map(r => (
                  <button key={r} onClick={() => setRoleFilter(r)}
                    className={`px-4 py-1.5 rounded-lg text-sm font-medium cursor-pointer capitalize transition-all ${
                      roleFilter === r ? 'bg-[#3d3d4d] text-white shadow-sm' : 'text-foreground hover:text-white/80'
                    }`}>
                ...
                {Array.from({ length: totalPages }, (_, i) => i + 1).map(p => (
                  <button key={p} onClick={() => setPage(p)}
                    className={`w-8 h-8 rounded-lg text-xs font-bold transition-all ${
                      p === page ? 'bg-primaryButton text-foreground' : 'text-foreground/35 hover:text-foreground hover:bg-foreground/5'
                    }`}>{p}</button>
                ))}
                      ...
                      return (
                        <button key={r.value} type="button"
                          ...
                          className={`relative flex flex-col items-start gap-0.5 p-3.5 rounded-xl border text-left transition-all ${
                            active ? r.sel : 'bg-foreground/3 border-foreground/[0.07] text-foreground/40 hover:bg-foreground/5'
                          }`}>
