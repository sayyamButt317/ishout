      ...
      <div className="min-h-[400px] flex flex-col justify-between">
        <div className="space-y-3">
          {subheader.map((row, rowIndex) => {
            ...
            return (
              <div key={rowIndex} className="flex items-start gap-4 w-full">
                <button
                  type="button"
                  onClick={() => toggleRowSelection(rowIndex)}
                  className={`shrink-0 w-6 h-6 rounded-full flex items-center justify-center transition-all mt-1 ${
                    isSelected
                      ? "bg-[#FF3B8D]"
                      : "bg-transparent border-2 border-white/30 hover:border-[#FF3B8D]"
                  }`}
                >
                  {isSelected && <Check className="w-4 h-4 text-black" />}
                </button>

                <div className="flex-1 rounded-2xl border border-white/10 bg-[#1A1A1A] p-5 hover:border-white/20 transition-colors">
                  {statuses && (
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="grid grid-cols-6 gap-x-4 gap-y-4">
                        {row.map((cell, cellIndex) => {
                          ...
                          return (
                            <div key={cellIndex} className={`min-w-0 ${shouldSpanTwo ? 'col-span-2' : ''}`}>
                              <p className="text-xs text-white/60 mb-1.5 truncate font-medium">
                        ...
                        {(showTrashIcon || showEyeIcon || showEditIcon) && (
                          <div className="min-w-0">
                            <p className="text-xs text-white/60 mb-1.5 font-medium">Action</p>
                            <div className="flex items-center gap-3">
                              {showEyeIcon && (
                                <Eye
                                  className="w-4 h-4 text-[#f7941D] cursor-pointer hover:opacity-80 transition-opacity"
                              ...
                              {showEditIcon && (
                                <Pencil
                                  className="w-4 h-4 text-blue-400 cursor-pointer hover:opacity-80 transition-opacity"
                              ...
                              {showTrashIcon && (
                                <Trash
                                  className="w-4 h-4 text-red-400 cursor-pointer hover:opacity-80 transition-opacity"
