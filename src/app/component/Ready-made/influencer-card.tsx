...
const InfluencerCard = ({
  ...
  const [actionLoading, setActionLoading] = useState<'approved' | 'rejected' | null>(
    null,
  );
  const [actionSuccess, setActionSuccess] = useState<'approved' | 'rejected' | null>(
    null,
  );
  ...
  const handleViewProfile = () => {
    const username = influencer?.username?.replace('@', '');
                ...
                <Badge className=" flex items-center gap-2 bg-transparent border text-xs">
                  {influencer?.platform === 'instagram' ? (
                    <>
                      ...
                      <span className="text-xs text-gray-500 font-thin">Instagram</span>
                    </>
                  ) : influencer?.platform === 'tiktok' ? (
                    <>
                      <SiTiktok className=" text-black dark:text-white" size={16} />
                      <span className="text-xs text-gray-500 font-thin">TikTok</span>
                    </>
                  ) : (
                    <>
                      <SiYoutube className=" text-red-500" size={16} />
                      <span className="text-xs text-gray-500 font-thin">YouTube</span>
                    </>
                  )}
                </Badge>
              </p>
      ...
      {actionSuccess === null && (showAccept || showReject) && (
        <div className="flex flex-col sm:flex-row gap-2 justify-center items-center w-full mt-4">
          ...
          {showDelete && (
            <Button
              variant="ghost"
              onClick={() => onDelete(influencer?.platform, influencer.influencer_id)}
